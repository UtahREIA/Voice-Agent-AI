...
- **Lender routing is differentiated by strategy → lender service (2026-08-10).** The active `investor_need='funding'` rows are one differentiated row per investor/asset type; their `vendor_categories` are the correct lender services **in priority order**, spelled in the exact `funding_financial` tokens the vendor records carry (`money_lender_private__hard_money`, `mortgage_broker`, `creative_finance_specialist`, `selfdirected_ira__401k_custodian`, `investment_advisor`, `loan_servicing_company`). E.g. `fix_and_flip` → hard money then creative; `buy_and_hold` → mortgage/DSCR then hard money; `passive_investing`/`syndication` → advisor/SDIRA; `commercial` (all commercial asset types via the `commercial` fallback) → hard money then mortgage then loan servicing. `resources.js` walks each row's categories **in order** so the primary lender surfaces first — don't revert that to vendor-order matching or the ordering dies. The differentiation lives entirely in the matrix because vendors carry no `investor_types`/asset data. Earlier granular rows were dead because they used category tokens (`hard_money_lender`) that don't fuzzy-match the vendor tokens (`money_lender_private__hard_money`); the live rows avoid that.
...
8. **Enroll the real lenders (Chris / Harmonie).** The lender routing matrix is differentiated and live, but the real lenders sit at `enroll_vendor_match=false` so they cannot surface — flips/holds route only to `TESTING` fixtures until they are vetted and enrolled through the human gate: **Conventus** (hard money), **Easy Street Capital** (hard money), **First Colony Mortgage** (mortgage broker). **CamaPlan** (SDIRA) is already enrolled and surfacing for passive/syndication. No code change needed once enrolled. Future data project: lender vendors carry no `investor_types`/asset-type data, so per-asset lender specialization (e.g. "self-storage lender") needs GHL tagging first.
